let gerenciaRedesSociais = () => {
  ...
  */
  ...
  for(let i=0; i<5; i++){ // para cada usuário do vetor
      for(let j=0; j<vetorUsuariosPost.lehsth; j++){ // usu[ario já existe
          if(vetorUsuarios[i].login == vetorUsuariosPost[i].login){
              // soma a qtde de posts de usuário
              vetorUsuariosPost[j].qtde = vetorUsuariosPost[j].qtde + vetorUsuarios[i]
              achou = true
            }
        }
    }
    // usuário ainda não existe no vetor vetorUsuariosPosts, então vamos criar
    if (!achou){// camos criar usuário no vetor
